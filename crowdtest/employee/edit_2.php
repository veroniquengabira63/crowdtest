  ...
  if(empty($_SESSION) && $_SESSION['user_type'] != 'EMPLOYEE'){
    header('Location: ../index.php');
			...
			if(!empty($image))
		{
			...
			mysqli_query($conn, "UPDATE tablet SET device_name='$Device',manufacture='$Manufacture',model='$Model',os='$OS ',browser='$Browser',version='$Version' WHERE id='$id'") or die(mysqli_error($conn));   
		}
		else
		{
			mysqli_query($conn, "UPDATE tablet SET device_name='$Device',manufacture='$Manufacture',model='$Model',os='$OS ',browser='$Browser',version='$Version' WHERE id='$id'");   

			header("location:all_device.php");
		
		}
    	}
    	$q=mysqli_query($conn,"SELECT * FROM tablet WHERE id='$id'");
