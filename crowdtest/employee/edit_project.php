...
if(isset($_POST["btnUpdate"]))
{
	...
	$Deadline             = $_POST["date"];//$_POST["date"];
	$Price              =$_POST["price"];   
	$Start_date  		= $_POST["start_date"];
	$Tester              =$_POST["tester_id"];   
	$Status           = $_POST["status"];

	mysqli_query($conn, "UPDATE project SET name='$Project_Name',description='$Description',date='$Deadline ',price='$Price',status='$Status',tester_id='$Tester',start_date='$Start_date' WHERE id='$id'") or die(mysqli_error($conn)); 

	header("location:all_projects.php"); 

}
$q=mysqli_query($conn,"SELECT * FROM project join signup ON project.tester_id = signup.id WHERE project.id=".$id);
			...
			<div class="col-md-8 col-md-offset-2">
				...
				<form method="post" enctype="multipart/form-data" class="form-horizontal" id="form">
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							<input  type="date" id="example1" name="date"  class="form-control input-lg " required="" value="<?php echo $data['date'] ?>" onfocus="(this.type='date')" onfocusout="(this.type='text')" placeholder="<?php echo $data['date'] ?>">
						</div>
					</div>

					<div class="form-group">
						<label for="example1" class="col-sm-2" control-label>Start Date</label>
						<div class="col-sm-8">
							<input  type="date" id="example1" name="start_date" class="form-control input-lg " required="" value="<?php echo $data['start_date'] ?>" onfocus="(this.type='date')" onfocusout="(this.type='text')" placeholder="<?php echo $data['start_date'] ?>">
						</div>
					</div>
					<div class="form-group">
					...
					<div class="form-group">
						<label for="status" class="col-sm-2" control-label>Project Assignee</label>
						<div class="col-sm-8">
							<select data-placeholder="Select Tester" class="form-control input-lg" name="tester_id" required="">
	                            <option></option>
	                             <?php
		                        	while($row = mysqli_fetch_object($tester)){
		                        ?>
		                            <option value="<?php echo $row->id ?>" <?php if($data["tester_id"] == $row->id) echo 'selected'; ?>><?php echo $row->first_name.' '.$row->last_name ?></option> 
		                        <?php 
		                        	}
		                        ?>
	                        </select>
